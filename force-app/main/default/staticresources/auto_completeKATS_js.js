AutoCompleteKATS = function(input,suggestions,lookupto,clearfunction,objectn)
{
	...
	function handleArrows(event){
		...
		try{
			...
			if (event.keyCode == 13 || event.keyCode == 9){ 
				...
				if (cursor != -1 && cursor < suggestions.childNodes.length && suggestions.childNodes[0].id != searchID && suggestions.childNodes[0].id != noResultsID){
				...
				if (event.keyCode == 40){ // DOWN arrow
					...
					if (cursor < suggestions.childNodes.length-1) {
						cursor ++;
						suggestions.childNodes[cursor].style.backgroundColor = bgColor;
					}
					if (cursor > 0)  {
						suggestions.childNodes[cursor-1].style.backgroundColor = "";
					}
